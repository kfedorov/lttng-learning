...
class ClusteringAutoencoder:
    ...
    def create_summaries(self):
        summaries = [tf.summary.scalar('total_cost', self.cost),
                     tf.summary.scalar('total_cost', self.base_cost),
    ...
    def save_embeddings(self, data, labelfunc):
        batch_size = 500
        output_dir = self.save_path + "_embed/"
        ...
        for i in range(n_iter):
            ...
            embeddings[start:end], decoded = self.sess.run([self.encoded_output, self.decoded_output],
                                                           feed_dict={self.input: data[start:end, :]})
            for j in range(batch_size):
                metadata_file.write('%06d\t%s\t%s\n' % (j+start, labelfunc(data[j+start]), labelfunc(decoded[j])))

        start = n_iter*batch_size
        embeddings[start:], decoded = self.sess.run([self.encoded_output, self.decoded_output],
                                                       feed_dict={self.input: data[start:, :]})
        ...
        summary_writer = tf.summary.FileWriter(output_dir, self.sess.graph)
        ...
        saver.save(self.sess, os.path.join(output_dir, 'model.ckpt'), self.global_step)
