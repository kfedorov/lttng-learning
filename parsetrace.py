...
class trace_parser:
    ...
    def parse(self):
        ...
        self.i_to_name = dict(enumerate(names))
        self.name_to_i = {v: k for k, v in self.i_to_name.items()}
        ...
        for event in col.events:
            ...
            windows[cur_window][self.comm_to_i.get(comm)][self.name_to_i.get(event.name)] += 1

        windows = np.divide(windows, self.window_size)
        self.windows = windows.reshape([window_count, len(commands) * len(names)])
    ...
    def windowToString(self, window, separator='\n'):
        ...
        window_shaped = window.reshape([len(self.comm_to_i), len(self.name_to_i)])
        for i,names in enumerate(window_shaped):
            used_names = names > 0
            if any(used_names):
                description += str(self.i_to_comm[i]) + separator
                used_names_idx = np.where(used_names)
                for name_idx  in used_names_idx[0]:
                    description += str(self.i_to_name[name_idx]) + ": " + str(names[name_idx]) + '; '
